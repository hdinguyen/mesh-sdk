...
class AgentSDK:
    ...
    def __init__(
        ...
        self.platform_url = os.getenv("PLATFORM_BASE_URL") or platform_url or "http://localhost:8000"
    ...
    def _validate_registration_fields(self) -> None:
        ...
        for field_name, field_spec in REQUIRED_REGISTRATION_FIELDS.items():
            if field_name not in registration_data or registration_data[field_name] is None:
                missing_fields.append(field_name)
            # Validate field value
            elif not self._validate_field_value(registration_data[field_name], field_spec):
    ...
    def _setup_acp_agent(self) -> None:
        ...
        @self.server.agent()
        async def agent_handler(input: list[Message], context) -> AsyncGenerator[Message, None]:
    ...
    def _wait_for_server_ready(self, timeout: int = 30) -> None:
        ...
        raise PlatformConnectionError(
            self.acp_base_url,
            "ACP server failed to start within timeout"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "capabilities" in error_data.get("error", ""):
                    raise AgentCapabilityError(self.capabilities, error_data.get("supported_capabilities"))
                ...
                raise PlatformUnavailableError(self.platform_url, response.headers.get("Retry-After"))
            else:
                raise PlatformConnectionError(self.platform_url, f"HTTP {response.status_code}: {response.text}")
